...
export default function Account() {
  ...
  const planTone = plan === "member" ? "text-[color:var(--accent)]" : plan === "admin" ? "text-purple-200" : "text-white";
  ...
  return (
    <div className="grid gap-6">
      <div className="rounded-2xl bg-[color:var(--card)] p-6 ring-1 ring-white/10">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="text-lg font-semibold text-white md:text-xl">我的</h1>
              <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-white/70">
                <span className="font-medium text-white">{displayName}</span>
                <span className="text-white/40">·</span>
                <span className="text-xs text-white/55">ID：{accountId}</span>
              </div>
              <div className={cn("mt-2 inline-flex items-center gap-2 rounded-xl bg-white/5 px-3 py-2 text-xs ring-1 ring-white/10", planTone)}>
      ...
      <div className="grid gap-4 md:grid-cols-[260px_1fr]">
        <aside className="rounded-2xl bg-[color:var(--card)] p-3 ring-1 ring-white/10">
          <div className="grid gap-1">
            {visibleSections.map((it) => (
              <button
                ...
                className={cn(
                  "flex items-center justify-between rounded-xl px-3 py-2 text-left text-sm ring-1 ring-white/10 transition",
            ...
            <button
              ...
              className="mt-2 inline-flex items-center justify-between rounded-xl bg-white/5 px-3 py-2 text-left text-sm text-white/75 ring-1 ring-white/10 transition hover:bg-white/10"
            >
              <span>退出</span>
        ...
        <div className="grid gap-4">
            ...
            <>
              <div className="rounded-2xl bg-[color:var(--card)] p-6 ring-1 ring-white/10">
                ...
                <div className="mt-5 flex flex-wrap items-center gap-2">
                  <Link
                    to="/pricing"
                    className="inline-flex items-center gap-2 rounded-lg bg-[color:var(--accent)] px-4 py-2 text-sm font-medium text-white transition hover:brightness-110"
                  >
                    <Crown className="size-4" />
                    管理订阅
                  </Link>
              ...
              <div className="rounded-2xl bg-[color:var(--card)] p-6 ring-1 ring-white/10">
                ...
                <div className="mt-4 grid gap-2">
                  {entRows.map((r) => (
                    <div key={r.label} className="flex items-center justify-between rounded-xl bg-white/5 px-4 py-3 ring-1 ring-white/10">
          ...
          {section === "unlocked" ? (
            <div className="rounded-2xl bg-[color:var(--card)] p-6 ring-1 ring-white/10">
              ...
              <div className="mt-4 grid gap-2">
                {grants.map((g) => (
                  <div key={g.strategyId} className="flex flex-col gap-2 rounded-xl bg-white/5 p-4 ring-1 ring-white/10 md:flex-row md:items-center md:justify-between">
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-white">{g.strategyId}</div>
                      <div className="mt-1 text-xs text-white/55">到期：{g.expiresAt ? formatDateTime(g.expiresAt) : "永久"}</div>
                    </div>
                    <button
                      type="button"
                ...
                {grants.length === 0 ? <div className="rounded-xl bg-white/5 p-4 text-sm text-white/60 ring-1 ring-white/10">暂无已解锁策略</div> : null}
              </div>
            </div>
          ) : null}
          ...
          {section === "security" ? (
            <div className="rounded-2xl bg-[color:var(--card)] p-6 ring-1 ring-white/10">
              ...
              <div className="mt-4 grid gap-2 text-sm text-white/70">
                <div className="rounded-xl bg-white/5 p-4 ring-1 ring-white/10">修改密码（待接入）</div>
                <div className="rounded-xl bg-white/5 p-4 ring-1 ring-white/10">两步验证 2FA（待接入）</div>
              </div>
            </div>
          ) : null}
          ...
          {section === "billing" ? (
            <div className="rounded-2xl bg-[color:var(--card)] p-6 ring-1 ring-white/10">
              ...
              <div className="mt-4 grid gap-2">
                <div className="rounded-xl bg-white/5 p-4 text-sm text-white/70 ring-1 ring-white/10">本模块在接入支付提供商后启用。</div>
                <Link to="/pricing" className="inline-flex w-fit items-center gap-2 rounded-lg bg-white/5 px-4 py-2 text-sm text-white/85 ring-1 ring-white/10 transition hover:bg-white/10">
